 ...
 */
static const char* BITCOIN_PID_FILENAME = "elektrond.pid";
/**
...
static void MaybeActivateAutomaticSnapshot(NodeContext& node)
{
    ...
    // snapshot.  This is necessary so that a node which bootstrapped from an
    // older checkpoint and then fell behind the pruned network can discard its
    // obsolete snapshot and load a newer one — exactly what a fresh node would
    // do.  We only proceed when the downloaded snapshot is strictly newer.
    {
        LOCK(::cs_main);
        if (Chainstate* current_cs = &chainman.CurrentChainstate();
            current_cs->m_from_snapshot_blockhash) {
            const CBlockIndex* existing_base = current_cs->SnapshotBase();
            const int existing_height = existing_base ? existing_base->nHeight : 0;
            const CBlockIndex* new_index = chainman.m_blockman.LookupBlockIndex(metadata.m_base_blockhash);
            const int new_height = new_index ? new_index->nHeight : 0;
            if (new_height <= existing_height) {
                // Same or older snapshot — nothing to do.
                return;
            }
            LogInfo("[snapshot] Old snapshot at %d will be replaced by newer snapshot at %d.\n",
                    existing_height, new_height);
            // Activation below will call DeleteChainstate() on the old one.
        }
    }
    ...
    if (!activation_result) {
        ...
        // Move the failed snapshot aside so we don't retry it endlessly.
        try {
            fs::rename(*snapshot_path, *snapshot_path + ".failed");
        } catch (const fs::filesystem_error& e) {
            LogWarning("[snapshot] Failed to rename failed snapshot: %s\n", e.what());
        }
        return;
    }
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogWarning("Relative datadir option '%s' specified, which will be interpreted relative to the "
                   "current working directory '%s'. This is fragile, because if Elektron Net is started in the future "
                   "from a different location, it will be unable to locate the current data files. There could "
                   "also be data loss if Elektron Net is started while in a temporary directory.",
